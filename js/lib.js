...
jQuery(document).ready(function(){
    ...
    empt = 'Vous devez indiquer votre nom';
    to_lng = 'Votre nom est trop long';
    to_lng = 'Votre nom est trop long';
    empt_mail = 'Votre adresse mail est trop courte';
    to_lng_mail = 'Votre adresse mail est trop longue';
    incor = 'Adresse mail incorecte';
    mes_empt = 'Vous devez indiquer votre message';
    to_lng_mes = 'Votre message est trop long';
	...
	function userSubmitFormResponse(response) {
		...
		if (rez.error == "") {
			jQuery(".sc_contact_form .result").addClass("sc_infobox_style_success").html("Votre message a été envoyé");
			setTimeout("jQuery('.sc_contact_form .result').fadeOut(); jQuery('.sc_contact_form form').get(0).reset();", 3000);
		} else {
			jQuery(".sc_contact_form .result").addClass("sc_infobox_style_error").html("Erreur de l'envoie " + rez.error);
		}
